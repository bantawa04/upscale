...
<script type="application/ld+json">
    {
      ...
      "description": "{{$tour->meta_description}}",
      "name": "{{$tour->name.' '.$tour->days.' Day(s)'}}",
      "image": "{{asset($tour->image->thumb)}}",
      "offers": {
        "@type": "Offer",
    ...
    <div class="row d-block">
                        ...
                        <tbody>
                            <tr>
                                <td class="align-middle">
                                    <span class="fa fa-barcode icon"></span>
                                </td>
                                <td>
                                    <div class="heading">Trip Code</div>
                                    <div class="data">ABC123</div>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                ...
                                <td>
                                    ...
                                    <div class="data">{{$tour->name}},
                                        {{$tour->region ? $tour->region->name : $tour->country->name}}</div>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                ...
                                <td>
                                    ...
                                    <div class="data">{{$tour->category->name}}</div>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="data">{{$tour->accomodation->name}}</div>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                ...
                                <td>
                                    ...
                                    <div class="data">{{$tour->meal->name}}</div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div class="col-lg-8 float-left">
            ...
            <div class="trip-detail">
                <ul id="trip-detail-nav" class="nav nav-pills nav-fill shadow-sm">
                    <li class="nav-item">
                ...
                <div id="trip-detail-overview" class="trip-detail-item">
                    <h3 class="mb-4">
                        {{$tour->name}}
                    </h3>
                    {!!$tour->overview!!}
                ...
                <div id="trip-detail-departures" class="trip-detail-item">
                    ...
                    <div>
                        The given cost are per person and exclude international flights. Given below are the departure
                        dates available for online booking. If the given date is not favorable then please write us and
                                    ...
                                    <div class="col-md col-6">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" id="year">
                                                @for ($i = date('Y'); $i <= date('Y')+3; $i++) <option value="{{$i}}">
                                                    {{$i}}</option>
                                                    @endfor
                                            </select>
                                        </div>
...
<div class="container mt-5 mt-md-4">
    <div id="section-similar-trips">
        <h3 class="text-center">Related Trips</h3>
        <div class="trips row">
...
<script>
    ...
    var $navbar = $('#navbar');
    var $tripDetailNav = $('#trip-detail-nav');

    var navbarHeight = $navbar.outerHeight();
    var tripDetailNavHeight = $tripDetailNav.outerHeight();
    ...
    $('body').scrollspy({
        target: '#trip-detail-nav',
        offset: navbarHeight + tripDetailNavHeight,
    });

    $tripDetailNav.find('.nav-item .nav-link').on('click', function (ev) {
        ev.preventDefault();
        var href = $(this).attr('href');
        $('html,body').animate({
            scrollTop: $(href).offset().top - (navbarHeight + tripDetailNavHeight) + 1,
        }, 'slow');
    });
});
...
$(function () {
    ...
    var $itinerary = $('#trip-detail-itinerary');
    var $collapsibleItinerary = $('#collapsible-itinerary');

    var toggleExpandCollapse = function () {

        var collapsed = $collapsibleItinerary.find('[data-toggle="collapse"].collapsed').length;
        if (collapsed === 0) {
            $itinerary.find('.btn-collapse').parent().removeClass('d-none');
            $itinerary.find('.btn-expand').parent().addClass('d-none');
        } else {
            $itinerary.find('.btn-collapse').parent().addClass('d-none');
            $itinerary.find('.btn-expand').parent().removeClass('d-none');
        }
    };

    $collapsibleItinerary.on('shown.bs.collapse', toggleExpandCollapse);
    $collapsibleItinerary.on('hidden.bs.collapse', toggleExpandCollapse);

    $itinerary.find('.btn-collapse').on('click', function (ev) {
        ev.preventDefault();
        $itinerary.find('.collapse').collapse('hide');
    });
    $itinerary.find('.btn-expand').on('click', function (ev) {
        ev.preventDefault();
        $itinerary.find('.collapse').collapse('show');
    });
});

$(function(){
    $("#quick-enquiry").submit(function(event){
	...
	$.ajax({
        ...
        success: function (data) {
			...
			{
				$("#response-msg").toggleClass('alert-danger alert-success');				
				$('.print-error-msg').find('ul').empty();
				$('.print-error-msg').css('display','block');
				$('.print-error-msg').append("<p>"+ data.message +"</p>");
				setTimeout(function() {
...
$(function() {
  $('.btn-book').on('click', function(e) {
    ...
    var offset = (windowHeight > formHeight)
        ...
        : 0;    
    e.preventDefault();
    $('html, body').animate({ scrollTop: $("#quick-enquiry").offset().top - offset}, 800, 'linear');
  });
});
$(function(){
    $('#search').click(function(e){
        ...
        $(".loading").show(), $.ajax({
            ...
            success: function(data) {
                $(".ajaxloadmoredeparture").html(data), $(".loading").hide()
            }
        })
    })
});
